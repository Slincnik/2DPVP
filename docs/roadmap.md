...
- [x] Разнести игроков по разным стартовым позициям.
- [x] Ввести состояния `waiting → countdown → active → finished`.
- [x] Добавить обратный отсчёт перед началом матча.
- [x] Запретить движение и атаки до состояния `active`.
- [x] Добавить ограничение времени матча.
- [x] Определить победителя по HP или оставшемуся HP после таймаута.
- [x] Надёжно доставлять `MatchStart` и `MatchEnd` через QUIC stream.
- [x] После результата закрывать room с небольшим grace period.
- [x] Добавить возврат клиента в меню.
- [x] Разрешить повторный поиск матча без перезапуска клиента.

### Disconnect

- [x] Обнаруживать разрыв соединения каждого игрока.
- [x] Определить правила технического поражения.
- [ ] Reconnect grace period отложен до P2 reconnect.
- [x] Не оставлять room/goroutine после ухода игроков.
- [x] Показывать клиенту причину завершения матча.

### Базовый gameplay UX

- [x] Отображать countdown, таймер, HP и результат.
- [x] Добавить визуальное состояние атаки.
- [x] Добавить эффект попадания.
- [x] Зафиксировать направленные атаки и реальные хитбоксы.
- [x] Добавить базовые звуки.
